...
export async function POST(request:Request,{params}:{params:Promise<{sellerId:string}>}){
  ...
  try{
    ...
    const {sanitizeSellerAvatar}=await import("@/lib/evidence");const safe=await sanitizeSellerAvatar(raw);path=`${sellerId}/${crypto.randomUUID()}.webp`;await uploadSellerImage(safe,path);
