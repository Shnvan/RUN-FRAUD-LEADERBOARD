...
export async function GET(){
  try {await requireAdmin();const [pending,corrections,sellers,audit,blocked]=await Promise.all([
    dbGet<unknown[]>("purchases","select=id,seller_id,purchase_date,quantity,unit_price,total_amount,flags,evidence_path,submitter_fingerprint,created_at,sellers(username)&moderation_status=eq.pending&order=created_at.asc&limit=100"),
    ...
    dbGet<unknown[]>("blocked_fingerprints","select=fingerprint,blocked_at,blocked_by&order=blocked_at.desc&limit=50")]);return Response.json({pending,corrections,sellers,audit,blocked},{headers:{"cache-control":"no-store"}});
