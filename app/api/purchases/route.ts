import { boundedFormData, dbGet, dbRpc, deleteEvidence, errorResponse, fingerprint, sameOrigin, uploadEvidence, verifyTurnstile, ServiceError } from "@/lib/server";
import { normalizeHandle, validHandle, validPrice, validPurchaseDate, validQuantity } from "@/lib/domain";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(idempotency)) throw new ServiceError(400,"Refresh the form and try again.");
    const prior = await dbGet<{id:string}[]>("purchases",`select=id&idempotency_key=eq.${idempotency}&limit=1`);
    ...
    if (file instanceof File && file.size) {
      ...
      evidencePath = `${crypto.randomUUID()}.${file.type.split("/")[1] === "jpeg" ? "jpg" : file.type.split("/")[1]}`;
    }
    await verifyTurnstile(token,new URL(request.url).hostname);
    const actorFingerprint = await fingerprint(request);
    const duplicateKey = await fingerprint(request,`${handle}|${date}|${quantity}|${price}`);
    if (evidencePath && file instanceof File) await uploadEvidence(file,evidencePath);
    const id = await dbRpc<string>("submit_purchase",{p_handle:handle,p_date:date,p_quantity:Number(quantity),p_unit_price:price,p_idempotency:idempotency,p_fingerprint:actorFingerprint,p_duplicate_key:duplicateKey,p_evidence_path:evidencePath});
