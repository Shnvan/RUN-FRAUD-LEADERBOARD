...
import {CharacterArt,BlinkSticker} from "@/components/old-web-art";
  ...
  return <SiteShell><main className="archive-desk">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
      <RetroWindow title={`CASE FILE / @${stats.username}`} tone="cobalt" bodyClassName="p-0" chrome="browser" overflow="visible">
        <div className="relative border-b-2 border-foreground bg-[var(--warm-paper)] p-5 text-[#251634] sm:p-7"><BlinkSticker kind="new"/><span className="archive-stamp ml-2">Seller profile</span><div className="mt-5 flex flex-wrap items-center gap-5">{stats.has_avatar?<Image unoptimized src={`/api/sellers/${stats.id}/avatar?v=${encodeURIComponent(stats.avatar_updated_at||"")}`} alt={`Moderator-selected reference image for @${stats.username}`} width={128} height={128} className="size-28 border-2 border-[#251634] object-cover sm:size-32"/>:<span className="grid size-28 place-items-center border-2 border-[#251634] bg-[var(--lime)] font-[family-name:var(--font-display)] text-5xl sm:size-32" aria-hidden="true">{stats.username.slice(0,1).toUpperCase()}</span>}<div className="min-w-0"><PrimaryProduct product={stats.primary_account_type}/><AccountTypeBadges types={stats.account_types||[]}/><h1 className="archive-heading mt-3 break-all">@{stats.username}</h1></div></div></div>
...
function SellerUnavailable(){return <SiteShell><main className="archive-desk grid min-h-[60dvh] place-items-center"><RetroWindow title="CASE FILES / UNAVAILABLE" tone="red" className="w-full max-w-lg" overflow="visible"><CharacterArt character="bozo" pose="confused" className="absolute -right-20 -top-20 hidden w-44 sm:block"/><h1 className="archive-heading">Records unavailable.</h1><p className="mt-3 text-sm">The data service is not connected right now.</p><Link href="/" className="archive-button archive-button--paper mt-5">Back to index</Link></RetroWindow></main></SiteShell>}
