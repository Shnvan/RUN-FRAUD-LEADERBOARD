...
export function HomeArchive({overview,loading,unavailable}:{overview:LossOverview;loading:boolean;unavailable:boolean}){
  return <div className="archive-desk">
    ...
    <RetroWindow title="QUICK FIND / REPORT DESK" tone="lime" className="mobile-quick-actions mt-3" bodyClassName="space-y-3">
      <SellerSearch hero id="mobile-seller-search"/>
      <RecordTrigger label="Report unresolved loss" compactLabel="Report" className="archive-button archive-button--red w-full"/>
    </RetroWindow>
    <div className="old-web-layout mt-3">
      <aside className="old-web-left space-y-3">
        <RetroWindow title="NAVIGATION.EXE" eyebrow="v1.0" tone="cobalt" className="nav-window" bodyClassName="space-y-2" overflow="visible">
          <ReactionImage name="court-jester" className="jester-hanger hidden lg:block"/>
      ...
      <div className="old-web-main min-w-0 space-y-4">
        <RetroWindow title="NETSCAPE FRAUS NAVIGATOR / MAIN STAGE" tone="red" bodyClassName="p-0" chrome="browser" overflow="visible" className="hall-window">
          <div className="circus-poster border-b-2 border-foreground p-4 text-[var(--classic-ink)] sm:p-6">
            <div className="circus-poster__copy" data-qa-protected="hero-copy">
              <div className="flex flex-wrap items-center gap-2"><span className="archive-stamp">Reviewed buyer reports</span><BlinkSticker kind="new"/></div>
              <p className="poster-kicker">THE INTERNET&apos;S CAREFULLY MODERATED</p>
              <h1 className="display-type mt-2 text-[clamp(1.8rem,5vw,4.75rem)]">WHO&apos;S RUNNING THE CIRCUS?</h1>
              <p className="mt-3 max-w-xl text-xs font-semibold leading-5 sm:text-sm">Top three sellers by buyer-reported amount still unresolved. Rankings change as approved open reports are updated.</p>
            </div>
            <div className="circus-poster__art" aria-hidden="true" data-qa-decoration="hero-art">
              <CharacterArt character="clown" pose="pointing" className="poster-fraus-sticker"/>
              <ReactionImage name="clown-juggling" priority className="main-dancing-clown"/>
      ...
      <aside className="old-web-right space-y-3">
        <RetroWindow title="REPORT DESK" tone="lime" overflow="visible" className="report-desk-window"><ReactionImage name="clown-waving" className="report-waving-clown hidden lg:block"/><p className="text-sm font-semibold">Have an unresolved purchase?</p><p className="mt-2 text-xs leading-5">Buyers can submit an itemized report without an account. Nothing appears here until moderator review.</p><RecordTrigger label="Open report desk" compactLabel="Report" className="archive-button archive-button--red mt-4 w-full"/></RetroWindow>
        <RetroWindow title="LIVE STATUS" tone="navy"><div className="flex items-start justify-between gap-2"><span className="archive-stamp text-foreground">{unavailable?"Offline":"Public index"}</span><BlinkSticker kind={unavailable?"warning":"new"}/></div><p className="mt-4 text-xs leading-5">Products and amounts shown here come only from approved, still-open reports.</p><div className="status-reactions"><ReactionImage name="red-angry"/><ReactionImage name="crying-face"/></div></RetroWindow>
        ...
        <RetroWindow title="SITE UPDATE" tone="cobalt"><div className="flex items-start gap-2"><div><p className="gallery-label">Last updated automatically</p><p className="mt-2 text-xs">Rankings refresh from reviewed reports. This decoration is not a fake visitor counter.</p></div><ReactionImage name="no-bozos" className="no-bozos-sticker"/></div><div className="mt-3 flex gap-1"><WebBadge name="guestbook"/><WebBadge name="email-receipts"/></div></RetroWindow>
      </aside>
    </div>
    ...
    <section id="gallery" className="leaderboard-scene mt-6 scroll-mt-4"><RetroWindow title="CASE FILES / FULL RANKED INDEX" tone="navy" bodyClassName="p-0" chrome="browser">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b-2 border-foreground bg-[var(--warm-paper)] px-4 py-3 text-[var(--classic-ink)]"><div><p className="gallery-label">Buyer report index</p><h2 className="display-type text-xl sm:text-2xl">Open case files</h2></div><span className="archive-stamp">PHP / ongoing</span></div>
      <div role="status" aria-live="polite">{loading?<div className="p-8 text-sm">Loading case files…</div>:unavailable?<EmptyState title="Index temporarily unavailable" detail="Please try again later." reaction="angry-yellow"/>:overview.leaders.length?overview.leaders.map((seller,index)=><LeaderboardRow key={seller.id} seller={seller} rank={index+1}/>):<EmptyState title="No reviewed reports yet" detail="Case files appear after moderator approval." reaction="sad-clown"/>}</div>
...
function EmptyState({title,detail,reaction}:{title:string;detail:string;reaction:"sad-clown"|"angry-yellow"}){return <div className="empty-reaction relative flex min-h-56 items-center gap-4 overflow-hidden p-6"><ReactionImage name={reaction} className="w-32 shrink-0"/><div className="relative z-10"><p className="display-type text-lg">{title}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div></div>}
...
function StageCard({rank,seller,loading}:{rank:number;seller?:SellerLossStats;loading:boolean}){
  ...
  const content=<div className={`relative flex h-full min-h-[260px] flex-col border-2 border-[var(--ink)] bg-[var(--card)] text-[var(--foreground)] ${primary?"md:min-h-[345px]":"md:min-h-[310px]"}`}>
    <div className={`flex items-center justify-between border-b-2 border-[var(--classic-ink)] px-3 py-2 font-[family-name:var(--font-pixel)] text-[10px] text-[var(--classic-ink)] ${primary?"bg-[#d7ec67]":rank===2?"bg-[#c7c8e7]":"bg-[#efbd9a]"}`}><span>{primary?"CENTER STAGE":"FEATURED FILE"}</span><span>#{String(rank).padStart(2,"0")}</span></div>
    <div className="flex flex-1 flex-col p-3">{seller?<><div className="flex items-start gap-2"><SellerAvatar seller={seller} size={primary?72:58}/><div className="min-w-0"><PrimaryProduct product={seller.primary_account_type}/><AccountTypeBadges types={seller.account_types||[]} limit={2}/></div></div><p className="mt-3 break-all text-xl font-bold leading-tight tracking-tight">@{seller.username}</p><div className="mt-auto grid grid-cols-2 gap-x-2 gap-y-3 border-t border-foreground/30 pt-3"><MiniMetric label="Unresolved" value={money(seller.unresolved_amount)}/><MiniMetric label="Reported value" value={money(seller.reported_purchase_value)}/><MiniMetric label="Accounts" value={count(seller.accounts_reported_purchased)}/><MiniMetric label="Open reports" value={count(seller.report_count)}/></div></>:<div className="flex flex-1 flex-col items-center justify-center text-center"><ReactionImage name="clown-waving" className="h-20 w-20"/><p className="gallery-label mt-3">{loading?"Loading…":"Unassigned"}</p><p className="mt-1 text-xs text-muted-foreground">Awaiting reviewed reports</p></div>}</div>
