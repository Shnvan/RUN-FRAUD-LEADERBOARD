...
export function SellerSearch({hero=false}:{hero?:boolean}) {
  ...
  function submit(event:FormEvent){event.preventDefault();const handle=normalizeHandle(value);if(!validHandle(handle)){setError("Enter a seller handle with 2–64 letters, numbers, dots, underscores, or hyphens.");document.getElementById(hero?"hero-seller-search":"nav-seller-search")?.focus();return}setError("");router.push(`/seller/${encodeURIComponent(handle)}`);}
  return <div className={hero?"min-w-0 w-full":"hidden lg:block"}><form onSubmit={submit} role="search" className={hero?"group flex h-14 min-w-0 w-full max-w-full items-center overflow-hidden border border-foreground bg-card p-1 transition focus-within:outline focus-within:outline-2 focus-within:outline-offset-2":"flex h-10 min-w-0 items-center border border-foreground/30 bg-card px-3"}>
    ...
    <input id={hero?"hero-seller-search":"nav-seller-search"} value={value} onChange={e=>{setValue(e.target.value);if(error)setError("")}} placeholder="Search seller handle" aria-invalid={Boolean(error)} aria-describedby={error?`${hero?"hero":"nav"}-search-error`:undefined} className={hero?"min-w-0 flex-1 bg-transparent px-2 text-[15px] outline-none placeholder:text-muted-foreground":"w-40 bg-transparent px-2 text-xs outline-none placeholder:text-muted-foreground"}/>
  ...
  </form>{error&&<p id={`${hero?"hero":"nav"}-search-error`} role="alert" className="mt-2 text-xs leading-5 text-foreground">{error}</p>}</div>;
}
